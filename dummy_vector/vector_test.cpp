...
TEST(DummyVector, Iterate) {
    // Итерирование по bmstu::dummy_vector
    {
        ...
        {
            bmstu::dummy_vector<int> v;
            EXPECT_EQ(v.begin(), v.end());  // Check if begin() is equal to end() for an empty vector
        }

        // Непустой вектор
        {
            bmstu::dummy_vector<int> v(10, 42);
            EXPECT_EQ(*v.begin(), 42);  // Dereference iterator to get the value
            EXPECT_EQ(v.end(), v.begin() + v.size());  // Check if end() is correct
        }
    }
}
